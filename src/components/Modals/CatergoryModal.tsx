...
const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        // Show the loading toast while uploading the image
        toast.promise(
          (async () => {
            ...
            return imageUrl; // Return image URL for success message
          })(),
          {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!category) {
        ...
        await axiosInstance.patch(`/productCategory/update/${category._id}`, formDataToSubmit, {
          headers: {
            "Content-Type": "application/json", // Ensure content type is JSON
          },
        });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end space-x-6 mt-8">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`px-3 py-1 text-lg bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 disabled:cursor-not-allowed ${loading ? 'cursor-wait' : ''
                }`}
            >
